...
def fix_schema(apps, schema_editor):
    ...
    with connection.cursor() as cursor:
        ...
        if 'created' in columns and 'modified' in columns:
            ...
            # SQLite doesn't support renaming columns in older versions, so we need to:
            ...
            try:
                # Drop old columns
                # SQLite doesn't support DROP COLUMN in older versions, so we'll leave them
                print("⚠ Cannot drop old columns in SQLite without recreating table - leaving them for now")
            except Exception as e:
